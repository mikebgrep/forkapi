...
class ShoppingListSerializer(serializers.Serializer):
    ...
    def update(self, instance, validated_data):
        ...
        if found_recipe.pk in instance.recipes:
            for x in instance.items.all():
                if x.name in [
                    z.name for z in found_recipe.ingredients.all()
                ] and x.quantity in [
                    z.quantity for z in found_recipe.ingredients.all()
                ]:
                    x.times += 1
                    x.save()
        else:
            for x in found_recipe.ingredients.all():
                ingredient_data = model_to_dict(
                    x, fields=["name", "quantity", "metric"]
                )
    ...
    def to_representation(self, instance):
        return SingleShoppingListSerializer(context=self.context).to_representation(
            instance
        )
