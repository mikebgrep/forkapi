...
class FilterRecipeByLanguage(DjangoFilterBackend):
    def filter_queryset(self, request, queryset, view):
        ...
        if translated_language:
            translated_recipes = [
                x for x in queryset if x.language == translated_language
            ]
            ...
            exclude_recipes = [
                ...
                for x in queryset
                if x.is_translated
                and x.language != translated_language
                and x.language != "English"
            ]

            non_translated_recipes = [
                x
                for x in queryset
                if x not in translated_recipes
                and len([y for y in translated_recipes if y.original_recipe_pk == x.pk])
                < 1
                and x not in exclude_recipes
            ]
