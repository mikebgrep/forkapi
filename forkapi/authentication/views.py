...
class SignUpView(generics.CreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        UserSettings.objects.create(
            user=instance, preferred_translate_language="English"
        )
...
class LoginView(generics.CreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        os.environ["DEFAULT_RECIPE_DISPLAY_LANGUAGE"] = (
            user.settings.preferred_translate_language or "None"
        )
...
class UpdateUserPasswordUsernameAndEmail(generics.UpdateAPIView):
    ...
    def put(self, request, *args, **kwargs):
        ...
        if not user.check_password(old_password):
            return Response(
                data={"errors": ["Old password does not match current password"]},
    ...
    def patch(self, request, *args, **kwargs):
        ...
        if not serializer.is_valid():
            ...
            error_msg = (
                "This email address already exists or is invalid.Please choice another."
                if "email" in errors and "username" not in errors
                else "This username already exists.Please choice another."
                if "email" not in errors and "username" in errors
                else "This username and the email already exists.Please choice another."
            )
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            if user:
                ...
                current_token = models.PasswordResetToken.objects.filter(
                    email=email
                ).first()
                password_reset_token = (
                    PasswordResetToken(email=email, token=token)
                ...
                return Response(
                    data={"token": password_reset_token.token},
    ...
    def post(self, request, *args, **kwargs):
        ...
        if password != confirm_password:
            return Response(
                data={
                    "errors": [
                        "Confirm new password and the new password does not match"
                    ]
                },
        ...
        if token:
            ...
            try:
                ...
                return Response(
                    data={"errors": ex.messages}, status=HTTP_400_BAD_REQUEST
                )
